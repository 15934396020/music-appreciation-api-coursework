...
\begin{titlepage}
    ...
    \begin{tcolorbox}[colback=white,colframe=secondary,boxrule=1pt,arc=4pt,width=0.92\textwidth,center]
    \centering
    \textbf{Submission Links (per Coursework Brief, Section 7a)}\\[4pt]
    \small
    \textbf{Public GitHub Repository:}\\
    \href{https://github.com/15934396020/music-appreciation-api-coursework}{\texttt{https://github.com/15934396020/music-appreciation-api-coursework}}\\[3pt]
    \textbf{API Documentation (hosted in the GitHub repository):}\\
    \href{https://github.com/15934396020/music-appreciation-api-coursework/blob/main/docs/API\_DOCUMENTATION.pdf}{\texttt{docs/API\_DOCUMENTATION.pdf}}\\[3pt]
    \textbf{Presentation Slides (hosted in the GitHub repository):}\\
    \href{https://github.com/15934396020/music-appreciation-api-coursework/blob/main/docs/PRESENTATION\_OPTIMIZED.pptx}{\texttt{docs/PRESENTATION\_OPTIMIZED.pptx}}\\[3pt]
    \textbf{Live Deployment:}\\
    \href{https://weidademiaoxiao.pythonanywhere.com}{\texttt{https://weidademiaoxiao.pythonanywhere.com}}
    \end{tcolorbox}
...
\textbf{Out of Scope:} Audio playback and external API integration (e.g., Spotify/Last.fm) were excluded to maintain a focused, coursework-appropriate prototype, though the architecture is designed to accommodate these in the future.
...
\begin{itemize}[label=\textcolor{primary}{\textbullet}, noitemsep]
    ...
    \item \textbf{Authentication:} API key-based authentication for all write operations, while read operations remain publicly accessible.
\end{itemize}

\subsection{Non-Functional Requirements}
\begin{itemize}[label=\textcolor{primary}{\textbullet}, noitemsep]
    \item \textbf{Simplicity and Maintainability:} Clean, modular code structure.
    \item \textbf{Reliability:} Comprehensive error handling with structured JSON error responses and input validation.
    \item \textbf{Testability:} High test coverage using automated test suites (55 tests across 9 test classes).
    \item \textbf{Documentation:} Auto-generated interactive API documentation (Swagger UI) and static PDF documentation.
    \item \textbf{Deployment:} Externally hosted ASGI deployment on PythonAnywhere, accessible at \url{https://weidademiaoxiao.pythonanywhere.com}.
\end{itemize}
...
\begin{enumerate}[label=\textbf{\textcolor{primary}{\arabic*.}}, noitemsep]
    ...
    \item \textbf{Authentication Layer:} API key validation via the \texttt{X-API-Key} header for write operations.
...
\begin{itemize}[label=\textcolor{primary}{\textbullet}, noitemsep]
    \item \textbf{Public endpoints (GET):} All read operations remain publicly accessible without authentication, allowing unrestricted browsing.
    \item \textbf{Protected endpoints (POST, PUT, DELETE):} All write operations require a valid API key passed via the \texttt{X-API-Key} header.
\end{itemize}

\subsection{Structured Error Handling}
A custom error handling module registers exception handlers for both HTTP exceptions and Pydantic validation errors. All error responses follow a consistent JSON structure, ensuring that API consumers receive predictable, machine-readable error responses regardless of the error type.

\subsection{Key Implementation Features}
\begin{itemize}[label=\textcolor{primary}{\textbullet}, noitemsep]
    \item \textbf{Lifespan Management:} The application uses FastAPI's modern \texttt{@asynccontextmanager} lifespan event to automatically create database tables and inject 29 seed tracks across 8 genres upon startup.
...
\begin{itemize}[label=\textcolor{primary}{\textbullet}, noitemsep]
    \item \textbf{Test Coverage:} The suite contains 55 tests across 9 test classes covering all endpoint groups (General, Genres, Tracks, Reviews, Tags, Collections, Analytics, Authentication, and Validation).
    \item \textbf{Test Isolation:} A custom \texttt{pytest.fixture} with \texttt{scope="session"} ensures the FastAPI lifespan event is triggered correctly during testing, initializing the database and seed data.
    \item \textbf{Authentication Testing:} Dedicated tests verify that write operations return 401 without an API key and 403 with an invalid key, while confirming that all GET endpoints remain publicly accessible.
...
\begin{enumerate}[label=\textbf{\textcolor{primary}{\arabic*.}}, noitemsep]
    \item \textbf{Single API Key:} The current authentication uses a single shared API key rather than per-user credentials. Implementing OAuth2 with JWT tokens would enable user-specific resource ownership and access control.
    ...
    \item \textbf{Rate Limiting:} No request rate limiting is currently implemented. Adding rate limiting would protect the API from abuse in a production setting.
\end{enumerate}

\section{Conclusion}

The Music Appreciation and Discovery API successfully demonstrates the design and implementation of a modern, database-backed web service. By leveraging FastAPI and SQLAlchemy, the project delivers a clean, modular, externally hosted, and fully tested RESTful API with 25 endpoints, API key authentication, structured error handling, and 5 analytical query endpoints. The live deployment at \url{https://weidademiaoxiao.pythonanywhere.com} strengthens the coursework submission by showing that the system is not only functional in local development but also accessible on an external web server. The inclusion of comprehensive testing (55 automated tests) and robust input validation ensures the system is resilient, explainable, and well aligned with the criteria for the XJCO3011 coursework.

\vspace{1em}

\begin{genaibox}
In accordance with the module's GREEN GenAI policy, generative AI tools (specifically ChatGPT and GitHub Copilot) were used as assistive tools throughout this project. 
